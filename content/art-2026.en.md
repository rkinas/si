...
**D. Test-time computing** – how to control model output in real time to obtain new, qualitatively better output, instead of just scaling parameters {{< cite "test_time_compute" >}}? In "Scaling LLM Test-Time Compute Optimally can be More Effective than Scaling Model Parameters," the authors introduce solutions allowing the generation of multiple answer proposals, which are then searched, evaluated, and selected as the generation result.
